...
def fallback_query_parser(query: str, domain: str) -> Tuple[List[str], str]:
    ...
    if domain == "chemistry":
        ...
        for common_name, formula in CHEM_DICTIONARY.items():
            ...
            if re.search(pattern, query_lower):
                ...
                for keyword in [
                    ...
                    if (
                        keyword_index != -1
                        and query_lower.find(common_name) > keyword_index
                    ):
                        is_goal = True
                        break
        ...
        for eq in raw_eqs:
            ...
            cleaned_eq = re.sub(
                r"^(find|given|solve|prove|tìm|cho)",
                "",
                cleaned_eq,
                flags=re.IGNORECASE,
            )
    ...
    try:
        ...
        class ExtractedProblem(BaseModel):
            initial_facts: List[str] = Field(
                ...,
                description="Clean formal logic strings or chemical formulas extracted from the context.",
            )
            goal_fact: str = Field(
                ...,
                description="The target final assertion or chemical compound to deduce.",
            )
        ...
        prompt = ChatPromptTemplate.from_messages(
            [("system", system_prompt), ("human", "Parse this query: {query}")]
        )

        logger.info(
            "Calling model-agnostic structured parser for domain '%s'...", domain
        )
        ...
        try:
            ...
            logger.info(
                "Model does not support with_structured_output or failed: %s. Using manual JSON parsing.",
                e,
            )
        ...
        logger.warning(
            "LangChain model-agnostic parser call failed: %s. Falling back to offline parser.",
            e,
        )
        return fallback_query_parser(query, domain)
...
def map_text_to_graph_fact(
    ...
    try:
        ...
        if domain_collection not in collections:
            logger.info(
                "Collection '%s' not found. Querying central collection '%s'.",
                domain_collection,
                collection_name,
            )
        ...
        logger.warning(
            "Failed to query collections: %s. Defaulting to '%s'.", e, collection_name
        )
        domain_collection = collection_name

    logger.info(
        "Mapping text '%s' using collection '%s' for domain '%s'",
        text,
        domain_collection,
        domain,
    )
    ...
    try:
        ...
        if results:
            ...
            logger.info(
                "Exact payload match (value) found for '%s' in '%s'",
                text,
                domain_collection,
            )
        else:
            # Try matching text against label field
            label_filters = must_filters + [
            ...
            if results:
                payload_match = results[0].payload
                logger.info(
                    "Exact payload match (label) found for '%s' in '%s'",
                    text,
                    domain_collection,
                )
    except Exception as e:
        ...
        try:
            ...
            if results.points:
                ...
                if match.score >= HIGH_CONFIDENCE_THRESHOLD:
                    ...
                    logger.info(
                        "High-confidence vector match (%0.4f >= %0.2f) found for '%s' in '%s'",
                        match.score,
                    ...
                    logger.info(
                        "Vector match found for '%s' but score %0.4f is below high-confidence threshold %0.2f",
                        text,
                        match.score,
                        HIGH_CONFIDENCE_THRESHOLD,
                    )
    ...
    if payload_match:
        ...
        if not neo4j_id and domain_collection != collection_name:
            try:
                central_results, _ = qdrant_client.scroll(
                    ...
                    scroll_filter=Filter(
                        must=[
                            ...
                            FieldCondition(
                                key="value", match=MatchValue(value=canonical_value)
                            ),
                        ]
                    ),
                    limit=1,
                ...
                logger.warning(
                    "Failed to resolve neo4j_id from central collection: %s", e
                )
        ...
        logger.info(
            "Successfully mapped '%s' to Fact value='%s', label='%s', neo4j_id='%s'",
            text,
            canonical_value,
            canonical_label,
            neo4j_id,
    ...
    logger.info(
        "Ad-hoc fallback routing for '%s' -> Value: '%s', ID: '%s'",
        text,
        mapped_val,
        neo4j_id,
    )
    return Fact(
        id=neo4j_id, value=mapped_val, domain=domain, attributes={"label": text}
    )
...
def route_query(query: str, domain: str) -> Tuple[List[Fact], Fact]:
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            if attempt == max_retries:
                logger.warning(
                    "Could not connect to Qdrant at %s:%d after %d attempts: %s. Using ad-hoc routing.",
                    qdrant_host,
                    qdrant_port,
                    max_retries,
                    e,
    ...
    for f_text in facts_text:
        if qdrant:
            ...
            # Offline fallback
            val = (
                CHEM_DICTIONARY.get(f_text.lower(), f_text)
                if domain == "chemistry"
                else f_text
            )
            mapped_facts.append(
                Fact(id=f"adhoc_{domain}_{abs(hash(val))}", value=val, domain=domain)
            )
    ...
    if qdrant and goal_text:
        ...
        val = (
            CHEM_DICTIONARY.get(goal_text.lower(), goal_text)
            if domain == "chemistry" and goal_text
            else goal_text
        )
        mapped_goal = Fact(
            id=f"adhoc_{domain}_{abs(hash(val))}", value=val, domain=domain
        )
    ...
    logger.info(
        "Neuro-Symbolic routing COMPLETE. Mapped %d Facts, Goal: %s",
        len(deduped_facts),
        mapped_goal.value,
    )
